...
(() => {
  const API = {
    cats: "/estimator/api/dje-categories",
    subs: (category) =>
      `/estimator/api/dje-subcategories?category=${encodeURIComponent(
        category || ""
      )}`,
    descs: (category, subcategory) =>
      `/estimator/api/dje-descriptions?category=${encodeURIComponent(
        category || ""
  ...
  const formatUSD =
    window.formatUSD || ((n) => `$${(Number(n) || 0).toFixed(2)}`);
  ...
  function rowEls(i) {
    ...
    return {
      ...
      costCell:
        document.querySelector(`#djeCost_${i}`) || tr?.cells?.[6] || null, // was 5
  ...
  function updateHeaderTotal() {
    ...
    $$(
      ...
      .concat(
        $$(
          "[id^='djeExt_']:not(#djeExt_0):not(#djeExt_1):not(#djeExt_2):not(#djeExt_3):not(#djeExt_4):not(#djeExt_5):not(#djeExt_6):not(#djeExt_7):not(#djeExt_8):not(#djeExt_9)"
        )
      )
  ...
  function djeCollectRowsFromDOM() {
    ...
    for (const tr of Array.from(tbody.rows)) {
      const anyDataEl = tr.querySelector("[data-row]");
      if (!anyDataEl) continue;
      ...
      const desc_id =
        tr.cells[3]?.querySelector("select.dje-desc")?.value || "";
      const qty = tr.cells[4]?.querySelector("input.cell-qty")?.value || "";
      const multi =
        tr.cells[5]?.querySelector("input.cell-multi")?.value || "1";
  ...
  function wireRow(i) {
    ...
    if (!catSel) {
      ...
      catSel.className = "dje-cat";
      catSel.setAttribute("data-row", String(i));
      catHost.appendChild(catSel);
    }

    let subSel = subHost.querySelector("select");
    if (!subSel) {
      subSel = document.createElement("select");
      subSel.className = "dje-sub";
      subSel.setAttribute("data-row", String(i));
      subSel.disabled = true;
      subHost.appendChild(subSel);
    }

    let descSel = descHost.querySelector("select");
    if (!descSel) {
      descSel = document.createElement("select");
      descSel.className = "dje-desc";
      descSel.setAttribute("data-row", String(i));
      descSel.disabled = true;
      descHost.appendChild(descSel);
    }
    ...
    catSel.addEventListener("change", async () => {
      ...
      subSel.disabled = true;
      populateSelect(descSel, []);
      descSel.disabled = true;
      if (costCell) costCell.textContent = formatUSD(0);
      const { extCell } = rowEls(i);
    ...
    subSel.addEventListener("change", async () => {
      ...
      populateSelect(
        descSel,
        descs.map((d) => ({
          id: String(d.id ?? d.description),
          label: d.description ?? String(d.id),
          cost: d.cost,
        })),
        ""
      );
  ...
  function ensureAutogrow(idx) {
    ...
    const currentIsLast =
      lastDataRow && Number(lastDataRow.getAttribute("data-row")) === idx;
  ...
  function wireReset() {
    ...
    btn.addEventListener("click", () => {
      ...
      const tbody = $("table tbody");
      while (tbody && tbody.rows.length > 10)
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      loadEstimateData?.();
      if (Array.isArray(estimateData?.costs?.dje_rows))
        savedRows = estimateData.costs.dje_rows;
    } catch {}
    ...
    (async () => {
      for (let i = 0; i < savedRows.length; i++) {
        ...
        if (multiEl) {
          multiEl.value = row.multi ?? "1";
          sanitizeMultiInput(multiEl);
        }
        ...
        if (desSel && row.cat_id && row.sub_id) {
          ...
          const hit = descs.find(
            (d) => String(d.id ?? d.description) === row.desc_id
          );
    ...
    lastTr
      ?.querySelector("input.cell-notes")
      ?.addEventListener("input", () => ensureAutogrow(lastIdx), {
        passive: true,
      });
    lastTr
      ?.querySelector("input.cell-qty")
      ?.addEventListener("input", () => ensureAutogrow(lastIdx), {
        passive: true,
      });
    lastTr?.querySelector("input.cell-multi")?.addEventListener(
      "input",
      () => {
