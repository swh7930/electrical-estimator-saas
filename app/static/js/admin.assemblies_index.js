(() => {
  ...
  (() => {
    ...
    const existingTbody = document.getElementById('compExistingBody');

    function renderExisting(list, asmId) {
      if (!existingTbody) return;
      existingTbody.innerHTML = '';
      if (!list || !list.length) {
        const tr = document.createElement('tr');
        tr.innerHTML = `<td colspan="5" class="text-center text-muted py-3">No components.</td>`;
    ...
    async function loadComponents(asmId) {
      ...
      try {
        ...
        renderExisting(data, asmId);
      } catch {
        existingTbody.innerHTML = `
          <tr><td colspan="5" class="text-center text-danger py-3">Failed to load components.</td></tr>
        `;
      }
    }
...
(() => {
  ...
  const compTbody = $('compStagedBody');
  ...
  // Add component to staged list (Qty = integer only)
