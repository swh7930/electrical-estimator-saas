...
def test_admin_unsubscribed_sees_banner_with_plans_cta(app, client):
    org, u = _org_user(app, role=ROLE_ADMIN, active=False)
    _login(client, u.id)
    resp = client.get("/")
...
def test_non_admin_unsubscribed_sees_contact_admin_banner(app, client):
    org, u = _org_user(app, role=ROLE_MEMBER, active=False)
    _login(client, u.id)
    resp = client.get("/")
    html = resp.get_data(as_text=True)
    assert "doesn’t have an active subscription" in html or "doesn't have an active subscription" in html
    assert ">View plans<" not in html

def test_admin_with_customer_id_sees_manage_billing_in_settings(app, client):
    org, u = _org_user(app, role=ROLE_ADMIN, active=True)
    _login(client, u.id)
